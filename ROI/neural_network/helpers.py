...
def detect_objects(model, loader, device, input_size, model_scale,
    ...
    total_coverage, n_done = 0, 0
    pbar = tqdm(loader, total=len(loader), desc=title)
    for imgs, _, _, gt_boxes, img_files, mask_files in pbar:
        ...
        pbar.set_postfix({'coverage': f'{total_coverage * 100 / n_done:.2f}%'})
